...
class BasePage(object):
    ...
    def watch_device(cls, keyword):
        ...
        cls.d.watcher.start()
        # cls.d.xpath.watch_background(interval=1.0)

    @classmethod
    def unwatch_device(cls):
        '''关闭watcher '''
        # cls.d.xpath.watch_stop()  # 停止监控
        cls.d.watcher.remove()
        cls.d.watcher.stop()
    ...
    def screenshot(cls):
        ...
        date_time = str(round(time.time() * 1000))
        screenshot_name = date_time + '.jpg'  # PNG会进测试报告
        # path = os.path.join(ReportPath().get_path(), screenshot_name)
        path = os.path.join(tmp_path, screenshot_name)
    ...
    def press_back_until_special_element(self, element, max_back=10, timeout=0.5):
        while not element.exists(timeout):
            self.d.press('back')
            max_back = max_back - 1
            if max_back == 1:
                break

    def press_back_to_home(self):
        self.press_back_until_special_element(self.d(resourceId='com.vova.android:id/home_container'))

    def find_element_by_child(self, text, className, child_className):
        '''通过父节点、兄弟节点、子节点查询元素，可自行换其他如resourceId等'''
        return self.d(text=text).sibling(className=className).child(className=child_className)

    @staticmethod
    def element_is_exists(elements, timeout=2):
        '''判断某元素是否存在'''
    ...
    @staticmethod
    def wait_element_exists(element):
        try:
    ...
    @classmethod
    def wait_element_then_screenshot(cls, element):
