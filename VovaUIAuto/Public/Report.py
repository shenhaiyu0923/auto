...
def backup_report(report_path, backup_path, time):
    ...
    if os.path.exists(report_path):
        try:
            tmp_path = os.path.join(backup_path, 'Report_' + time)
...
def left_three_report(report_path):
    '''删除历史报告,保留最近三份'''
    if os.path.exists(report_path):
        try:
            files = [file for file in os.listdir(report_path)]
            for i in range(len(os.listdir(report_path)) - 3):
                path = os.path.join(report_path, files[i])
                shutil.rmtree(path)
        except PermissionError as e:
            raise e
...
if __name__ == '__main__':
    ...
    backup_report(r'E:\vova_uitest\TestSuite_mainflow\TestReport', r'E:\vova_uitest\TestSuite_mainflow\TestReport_History', now)
