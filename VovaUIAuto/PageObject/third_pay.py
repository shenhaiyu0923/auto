...
class third_pay_page(BasePage):
    @teststep
    def wait_web_page(self):
        try:
            #  if self.d(resourceId="com.vova.android:id/title_text").wait(timeout=15):
            if self.d(resourceId="com.vova.android:id/title_text", textContains='http').wait(timeout=15):
                pass
            else:
                raise Exception('Not in web_page')
        except Exception:
            raise Exception('Not in web_page')
